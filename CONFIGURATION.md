...
- `--db <path>` - one-off SQLite store location. **Flag wins over `LAST30DAYS_DB_PATH` env var wins over `.env` wins over the default `~/.local/share/last30days/research.db`.** An explicit DB also wins over the `--save-dir` scoped store (`<save-dir>/research.db`), so an orchestrator that exports both a memory dir and a dedicated DB keeps the DB. Useful for orchestrators that fan out many engine calls and want per-engagement persistence without polluting the shared store. Findings are persisted only when `--store` (or `LAST30DAYS_STORE=1`) is also active; the discovery-queue commands and the prior-run library context read the same DB on their own. `briefing.py` and `watchlist.py` read `LAST30DAYS_DB_PATH` from the process environment only.
...
**Optional frontmatter.** A `.md` or `.txt` file may start with a `---` block of `key: value` lines (flat values, one-level `{a: 1}` maps, `[a, b]` lists; no YAML library is involved; the block counts only when it contains at least one recognized key, so a note that opens with a horizontal rule is untouched). Recognized keys: `published_at` (ISO date or datetime, judged on the UTC calendar day; replaces the file modification time for the recency window), `url` (an `http(s)` URL shown as a link in the "From your files" block and used to collapse syndicated copies of one article; the item keeps its opaque `corpus://` key on purpose so private text never fuses with the public copy of the same article; the match transfers the file's ranking vote, capped at one top-rank vote, to the public copy and tags it `in your files` on screen only, never in saved or published output), `title`, `author`, `container`, `engagement` (a map of non-negative finite counts such as `{comments: 3, views: 120}` that feeds the engagement signal), and `tags` (a list, added to the text used for relevance matching). Unrecognized keys are kept under `metadata.frontmatter`, minus anything credential-shaped (`token`, `secret`, `password`, `cookie`, `api_key`, `auth`); that metadata reaches the saved raw report and `--json-profile=raw`, so a bridge should redact before writing. Files without frontmatter behave exactly as before. Two practical rules for bridge scripts: set each file's modification time to its `published_at` (the 500-file scan cap keeps the newest files by modification time before the date is read), and write immutable files (the text cache keys on modification time and size). This is what lets bridge scripts flatten feeds, mailing lists, and exported chats into a corpus directory with real dates and links; the privacy rules above still apply to every file.
...
| GitHub trending | none; `GITHUB_TOKEN` optional (raises the search rate limit from 10 to 30 requests per minute) | **opt-in, off by default**: `--search github,github_trending` per run or `INCLUDE_SOURCES=github_trending`. Two labeled halves, never blended: repositories rising this week from the public `github.com/trending` page (kept only when they lexically match the topic, all of them for a plain "what is trending" question) and new repositories in the topic from `/search/repositories` with `created:>=` (never `pushed:>`, which returns all-time giants), dropping repositories under 5 stars so a person or brand topic does not pull in name-collision junk. `EXCLUDE_SOURCES=github_trending` wins. | yes (public page and keyless API) |
...
| Telegram | `--telegram-sources=<handles>` **or** `TELEGRAM_SOURCES=<handles>` + `INCLUDE_SOURCES` contains `telegram`; `SCRAPECREATORS_API_KEY` optional (with it ScrapeCreators is the backend, without it the keyless `t.me/s` preview pages are; pin either with `LAST30DAYS_TELEGRAM_BACKEND=scrapecreators\|keyless`) | **opt-in, off by default**; public channel posts only (no keyword discovery). `--telegram-sources=aipost,durov` (or `TELEGRAM_SOURCES` env) auto-activates for that run like `--trustpilot-domain`. Accepts bare handle, `@handle`, `t.me/URL`, or `t.me/s/URL`; rejects joinchat links and numeric -100 IDs. `INCLUDE_SOURCES=telegram` or `--search telegram` without a channel list does not fetch. `EXCLUDE_SOURCES=telegram` wins. `TELEGRAM_MAX_PAGES` overrides page cap (quick=1, default=3, deep=6). Never on Recommended onboarding tier. | keyless: yes (public preview pages, no key). ScrapeCreators: 1 credit per live posts page; 10K free calls |
...
| `LAST30DAYS_TELEGRAM_BACKEND` | Pins the Telegram backend (`scrapecreators` / `keyless`). Unpinned: ScrapeCreators when its key is present, else the keyless `t.me/s` preview pages. A `scrapecreators` pin without the key is an explicit error (the source returns no items and the run marks Telegram as failed); unset the pin or pin `keyless` to use the preview pages. The keyless backend fetches at most 20 pages per channel per run with a short pause between pages. |
