...
| `LAST30DAYS_TELEGRAM_BACKEND` | Pins the Telegram backend (`scrapecreators` / `keyless`). Unpinned: ScrapeCreators when its key is present, else the keyless `t.me/s` preview pages. A `scrapecreators` pin without the key falls back to keyless with a log line. |
