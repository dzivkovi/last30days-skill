...
**Optional frontmatter.** A `.md` or `.txt` file may start with a `---` block of `key: value` lines (flat values, one-level `{a: 1}` maps, `[a, b]` lists; no YAML library is involved). Recognized keys: `published_at` (ISO date or datetime; replaces the file modification time for the recency window), `url` (an `http(s)` URL replaces the opaque `corpus://` key so fusion can merge the item with the same article seen on the web; other schemes are ignored), `title`, `author`, `container`, `engagement` (a map of non-negative counts such as `{comments: 3, views: 120}` that feeds the engagement signal), and `tags` (a list, added to the text used for relevance matching). Unrecognized keys are kept under `metadata.frontmatter`. Files without frontmatter behave exactly as before. This is what lets bridge scripts flatten feeds, mailing lists, and exported chats into a corpus directory with real dates and links; the privacy rules above still apply to every file.
