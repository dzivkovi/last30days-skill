...
- Every `lib/*.py` call to `log.source_log(...)` must pass `tty_only=False`. The default is `True`, which silently drops every line when stderr isn't a TTY (Claude Code, Codex, CI, captured output) — turning source observability into invisible failure. Enforced by `tests/test_source_log_visibility.py`. Background: dzivkovi/last30days-skill#13.
- Every `search_*` / `fetch_*` entry point under `lib/*.py` that catches `HTTPError` or `Exception` inline must either re-raise or return a value carrying an `error` key (e.g. `return {"items": [], "error": f"{type(e).__name__}: {e}"}`). The pipeline reads this envelope via `_swallowed_error_artifact()` and routes failures into `bundle.errors_by_source` so they surface in the rendered `## Source Errors` block. Without the envelope, the failure is invisible to end users — it looks identical to "0 items, API healthy." Enforced by an AST scan in `tests/test_source_error_propagation.py::SourceErrorEnvelopeConventionTests`.
